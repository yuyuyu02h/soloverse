...
2026-09-23: オンボーディングUIを変えず、`admired`を裏側の有名人モードへ分離した。ユーザー投稿ごとに`celebrity_scenes`を1件作り、いいね・コメント総数はローカルの成長曲線で演出する。AIは観客全体の空気と代表コメント8件を1回で生成し、名前・アイコン・時刻はローカルで付与する。通常リアクション、取りこぼし返信救済、住人自動増加はこのモードでは停止する。`celebrity_comments`は表示サンプルであり、大量アカウントや大量`likes`行は作らない。
